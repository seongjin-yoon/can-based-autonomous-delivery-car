 ...
 *
 * [목적지별 경로]
 *   A: 1번교차로 좌회전  → ArUco ID=1
 *   B: 1번교차로 우회전  → ArUco ID=2
 *   C: 1번교차로 직진 → 2번교차로 좌회전 → ArUco ID=3
 *   D: 1번교차로 직진 → 2번교차로 우회전 → ArUco ID=4
 *
 * 컴파일:
...
#define LINE_TH          0.06f
#define HIGH_TH          0.25f
...
// MQTT로 설정되는 값들
static std::atomic<int>  g_destination(0);    // 0=미설정, 1=A, 2=B, 3=C, 4=D
static std::atomic<bool> g_start_cmd(false);  // 출발 신호
...
static uint8_t get_junction_action(int dest, int junc_count) {
    ...
    //  D: 1번→직진, 2번→우회전
    switch(dest) {
        case 1: if (junc_count == 0) return DIR_LEFT;    break; // A
        case 2: if (junc_count == 0) return DIR_RIGHT;   break; // B
        case 3:
            if (junc_count == 0) return DIR_FORWARD;           // C: 1번직진
            if (junc_count == 1) return DIR_LEFT;              // C: 2번좌회전
            break;
        case 4:
            if (junc_count == 0) return DIR_FORWARD;           // D: 1번직진
            if (junc_count == 1) return DIR_RIGHT;             // D: 2번우회전
            break;
    }
    return DIR_FORWARD;
}

// 목적지 ArUco ID (A=1, B=2, C=3, D=4)
                                   ...
                                   float& dbg_L, float& dbg_M, float& dbg_R)
{
    ...
    if (hasL && hasM && hasR && dbg_L >= HIGH_TH && dbg_R >= HIGH_TH) return J_PLUS;
    if (hasL && hasR && !hasM && (dbg_L >= HIGH_TH || dbg_R >= HIGH_TH)) return J_T;
    if (hasL && hasM && !hasR && dbg_L >= HIGH_TH) return J_T;
    if (hasM && hasR && !hasL && dbg_R >= HIGH_TH) return J_T;
    return J_NONE;
}
...
public:
    ...
    bool start(const std::string& host, int port = 1883) {
        ...
        if (rc != MOSQ_ERR_SUCCESS) {
            fprintf(stderr, "[MQTT] 연결 실패 rc=%d - 오프라인 모드로 실행\n", rc);
            return false;
        }
        loop_start();
...
protected:
    ...
    void on_message(const struct mosquitto_message* msg) override {
        ...
        try {
            ...
            // ★ 출동 명령 수신 (delivery/command/vehicle_001)
...
int main(int argc, char* argv[]) {
    ...
    while (1) {
        ...
        case S_FOLLOW: {
            ...
            if (dest > 0 && aid == get_target_aruco(dest)) {
            ...
            if ((t - last_tx) >= 200) {
                ...
                printf("\r[%s err=%4d] DEST:%d JUNC:%d RPM:%.0f",
                       d, err, dest, junc_count, g_rpm);
                fflush(stdout);
            }
            break;
        }
        ...
        cv::putText(vis, std::string("DEST:")+dn, cv::Point(10, 45),
                    cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0,255,0), 2);

        char info[32];
        snprintf(info, sizeof(info), "J:%d RPM:%.0f", junc_count, g_rpm);
