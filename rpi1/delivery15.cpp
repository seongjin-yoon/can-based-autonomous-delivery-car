...
#define UTURN_RPM         350.0f   // ★ 유턴은 더 강하게
...
int main(int argc, char* argv[]) {
    ...
    // 실제 카메라 해상도 출력
    printf("Actual camera resolution: %.0f x %.0f\n",
    ...
    while (1) {
        ...
        case S_UTURN: {
            ...
            printf("\r[U턴 중] %.1fs 경과 (라인 대기중)  ", dt / 1000.0f);
            fflush(stdout);

            // ★ 최소 2000ms 회전 후 라인 감지되면 완료 / 타임아웃 10초
            if (dt >= 2000) {
                int H2 = frame.rows, W2 = frame.cols;
                int y02 = (int)(H2 * 0.65);
                cv::Mat roi2 = frame(cv::Rect(0, y02, W2, H2 - y02));
