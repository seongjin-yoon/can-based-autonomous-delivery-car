/*
 * delivery_mqtt_merge10.cpp
 * 라인트레이싱 + ArUco + MQTT 통합 버전
 *
 * 수정사항 (merge10):
 *   1. VehicleMQTT 클래스를 recv_can 앞으로 이동 (컴파일 오류 수정)
 *   2. recv_can(sock) → recv_can(sock, mqtt) 인자 수정
 *   3. MQTT 구독 토픽 /destination → /order 수정
 *   4. last_junc = t → last_junc = 0 (첫 교차로 쿨다운 제거)
 *   5. 유턴 후 복귀 시도 last_junc = 0 (복귀 첫 교차로도 즉시 인식)
 *
 * 컴파일:
 *   g++ delivery_mqtt_merge10.cpp -o delivery10 \
 *       $(pkg-config --cflags --libs opencv4) \
 *       -lmosquittopp -lpthread
 * 실행:
 *   ./delivery10 [broker_ip]
 */
...
// ===================== 주행 파라미터 =====================
#define PRE_TURN_FORWARD_MS  600   // 좌/우회전 전 교차로 안으로 진입
#define DEFAULT_RPM       50.0f
#define TURN_RPM          120.0f
#define UTURN_RPM         200.0f   // ★ 유턴은 더 강하게
...
#define DUR_UTURN_MS          3000
#define DUR_REACQ_MS          1500   // ★ 500 → 1500ms (회전 후 라인 정렬 시간 확보)
#define DUR_REACQ_UTURN_MS    1000
#define REACQ_RPM             30.0f  // ★ 재조정 시 저속
...
typedef enum {
    ...
    S_JUNC_RIGHT,
    S_REACQUIRE,
    S_REACQ_STABLE,        // ★ REACQ 완료 후 직진 안정화
    S_REACQUIRE_AFTER_UTURN,
    S_DELIVER_WAIT,
    S_PRE_UTURN,   // ★ 도어 닫힘 후 직진 → 유턴 준비
    S_UTURN,
    S_FINISHED
} State;
...
static uint8_t        g_reacq_dir   = DIR_FORWARD; // ★ REACQUIRE 진입 방향
...
void send_cargo_info(int sock, const std::string& pin, int dest) {
    ...
    write(sock, &frame, sizeof(frame));
    printf("[CAN TX] 0x012 배달정보 전송 PIN=%s DEST=%d\n", pin.c_str(), dest);
}
...
// ===================== CAN 수신 =====================
// ★ VehicleMQTT 클래스 선언 후에 위치 + mqtt 인자 추가
        ...
        if (read(sock, &frame, sizeof(frame)) > 0) {
            if (frame.can_id == CAN_DOOR_STATUS && frame.data[0] == 0x00) {
                ...
                json msg = {
                    ...
                    {"order_id",    g_delivery_id},
                    {"event",       "pin_success"},
                    {"message",     "PIN 인증 성공 - 적재함 열림"},
                    {"timestamp",   get_timestamp()}
                };
                mqtt.pub("delivery/vehicle/" + g_vehicle_id + "/alert", msg, 1);
                printf("[MQTT] 인증 성공 알림 전송\n");
            }
                ...
                json msg = {
                    ...
                    {"order_id",    g_delivery_id},
                    {"event",       "pin_locked"},
                    {"message",     "PIN 5회 실패 - 적재함 잠금"},
                    {"timestamp",   get_timestamp()}
                };
                mqtt.pub("delivery/vehicle/" + g_vehicle_id + "/alert", msg, 1);
                printf("[MQTT] 잠금 알림 전송\n");
            }
        }
    }
}
...
int main(int argc, char* argv[]) {
    ...
    while (1) {
        ...
        case S_WAIT_CMD:
            ...
            if (g_start_cmd) {
                ...
                returning     = false;
                start_ts      = t;
                last_junc     = 0;   // ★ 첫 교차로 쿨다운 없이 바로 인식
                state         = S_FOLLOW;
                state_ts      = t;
                const char* dn = (dest==1)?"A":(dest==2)?"B":(dest==3)?"C":"D";
                printf("\n[출발] 목적지: %s\n", dn);
        ...
        case S_FOLLOW: {
            ...
            }
            {
                ...
                if (t - last_log >= 500) {
                    last_log = t;
                    const char* d2  = (cmd==DIR_FORWARD)?"STRAIGHT":(cmd==DIR_LEFT)?"LEFT":"RIGHT";
                    const char* dn2 = (dest==1)?"A":(dest==2)?"B":(dest==3)?"C":(dest==4)?"D":"?";
                    printf("\n[LOG %s] DIR:%-8s err:%4d | DEST:%s %s | JUNC:%d RPM:%.0f streak:%d L=%.2f M=%.2f R=%.2f",
                           get_timestamp().c_str(),
                           d2, err, dn2, returning?"RET":"GO",
                           junc_count, g_rpm, streak, rL, rM, rR);
                    fflush(stdout);
                }
            }
            break;
        ...
        case S_JUNC_STRAIGHT:
            ...
            if (dt >= DUR_STRAIGHT_MS) {
                g_reacq_dir = DIR_FORWARD; // 직진에서 왔으므로 직진 유지
                state    = S_REACQUIRE;
                state_ts = t;
                printf("\n[직진 완료] 라인 재탐색\n");
            }
    ...
    if (dt < PRE_TURN_FORWARD_MS) {
        ...
        printf("\r[좌회전 진입] %.1f/%.1fs forward      ",
               dt/1000.0f, PRE_TURN_FORWARD_MS/1000.0f);
        fflush(stdout);
        break;
    }

    // 2단계: 본격 좌회전
    uint64_t turn_dt = dt - PRE_TURN_FORWARD_MS;
    ...
    int H2 = frame.rows, W2 = frame.cols;
    int y02 = (int)(H2 * 0.55);   // 기존 0.65 -> 0.55 로 확대
    cv::Mat roi2 = frame(cv::Rect(0, y02, W2, H2 - y02));

    cv::Mat gray2, blur2, bin2;
    cv::cvtColor(roi2, gray2, cv::COLOR_BGR2GRAY);
    cv::GaussianBlur(gray2, blur2, cv::Size(7,7), 0);
    cv::threshold(blur2, bin2, 0, 255, cv::THRESH_BINARY_INV | cv::THRESH_OTSU);
    cv::morphologyEx(bin2, bin2, cv::MORPH_OPEN,
        cv::getStructuringElement(cv::MORPH_RECT, cv::Size(3,3)));

    std::vector<std::vector<cv::Point>> ctrs2;
    cv::findContours(bin2, ctrs2, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);

    double max_area2 = 0;
    int best_i = -1;
    for (int i = 0; i < (int)ctrs2.size(); i++) {
        double a = cv::contourArea(ctrs2[i]);
        if (a > max_area2) {
            max_area2 = a;
            best_i = i;
        }
    ...
    bool phase2  = (turn_dt >= 1300);   // 기존 2500 -> 1300 정도로 축소
    bool area_ok = (max_area2 >= 1200);

    bool cx_ok = false;
    if (area_ok && best_i >= 0) {
        cv::Moments mm = cv::moments(ctrs2[best_i]);
        if (mm.m00 > 0) {
            int cx2 = (int)(mm.m10 / mm.m00);
            // 좌회전 후 라인은 왼쪽~중앙 정도
            cx_ok = (cx2 < W2 * 0.65f);
        }
    }

    bool line_ok = phase2 && area_ok && cx_ok;

    printf("\r[좌회전] %.1fs area:%.0f %s      ",
           turn_dt/1000.0f, max_area2,
           !phase2 ? "warmup" : (line_ok ? "LINE!" : "searching..."));
    fflush(stdout);

    if (line_ok) {
        printf("\n[좌회전 완료] 라인 감지 (area=%.0f, %.1fs)\n",
               max_area2, turn_dt/1000.0f);
        g_reacq_dir = DIR_LEFT;
        state    = S_REACQUIRE;
        state_ts = t;
    }

    // 너무 오래 돌면 안전하게 REACQUIRE로 넘김
    if (turn_dt >= 5000) {
        printf("\n[좌회전 타임아웃] 강제로 REACQUIRE 진입\n");
        g_reacq_dir = DIR_LEFT;
        state    = S_REACQUIRE;
        state_ts = t;
    }
    break;
         ...
         case S_JUNC_RIGHT: {
            ...
            }
            {
                ...
                cv::findContours(bin2, ctrs2, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);
                double max_area2 = 0;
                for (auto& c : ctrs2) max_area2 = std::max(max_area2, cv::contourArea(c));

                bool phase2  = (dt >= 2500); // ★ 1500→2500ms 워밍업 연장
                bool area_ok = (max_area2 >= 1500); // ★ area 상한 제거

                bool cx_ok = false;
                if (area_ok && !ctrs2.empty()) {
                    int best_i = 0; double ba = 0;
                    for (int i = 0; i < (int)ctrs2.size(); i++) {
                        double a = cv::contourArea(ctrs2[i]);
                        if (a > ba) { ba = a; best_i = i; }
                    }
                    cv::Moments mm = cv::moments(ctrs2[best_i]);
                    if (mm.m00 > 0) {
                        int cx2 = (int)(mm.m10 / mm.m00);
                        // 우회전 후 라인은 화면 오른쪽~중앙에 있어야 함
                        cx_ok = (cx2 > W2 * 0.3f);
                    }
                }
                bool line_ok = phase2 && area_ok && cx_ok;

                printf("\r[우회전] %.1fs area:%.0f %s      ",
                       dt/1000.0f, max_area2,
                       !phase2 ? "warmup" : (line_ok ? "LINE!" : "searching..."));
                fflush(stdout);

                // ★ 라인 감지 시에만 REACQUIRE 전환 (타임아웃 없음 - 라인 찾을때까지 계속 회전)
                if (line_ok) {
                    printf("\n[우회전 완료] 라인 감지 (area=%.0f, %.1fs)\n", max_area2, dt/1000.0f);
                    g_reacq_dir = DIR_RIGHT;
                    state    = S_REACQUIRE;
                    state_ts = t;
                }
            }
        ...
        case S_REACQUIRE: {
            ...
            std::vector<std::vector<cv::Point>> ctrs2;
            cv::findContours(bin2, ctrs2, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);
            double max_area2 = 0; int best2 = -1;
            for (int i = 0; i < (int)ctrs2.size(); i++) {
                double a = cv::contourArea(ctrs2[i]);
                if (a > max_area2) { max_area2 = a; best2 = i; }
            }

            bool line_found = (max_area2 >= 800);
            int  err2 = 9999;
            bool centered = false;
            uint8_t reacq_turn_dir = g_reacq_dir; // 기본은 진입 방향
            if (line_found && best2 >= 0) {
                cv::Moments mm = cv::moments(ctrs2[best2]);
                int cx2 = (mm.m00 != 0) ? (int)(mm.m10 / mm.m00) : W2/2;
                err2 = cx2 - W2/2;
                centered = (abs(err2) <= 50);  // ★ 80 → 50px 엄격하게
                // ★ 라인 보이면 err 부호로 방향 판단 (라인 많은 쪽으로)
                reacq_turn_dir = (err2 > 0) ? DIR_RIGHT : DIR_LEFT;
            }

            // ★ 중앙 정렬 연속 카운터 (static - 상태 내에서 유지)
            static int center_streak = 0;
            if (centered) center_streak++;
            else          center_streak = 0;

            if ((t - last_tx) >= 50) {
                if (!line_found) {
                    // 라인 없음 → g_reacq_dir 방향으로 회전 (라인 찾는 중)
                    send_cmd(sock, g_reacq_dir, REACQ_RPM);
                } else if (!centered) {
                    // 라인 보임 → err 부호 방향으로 정렬
                    send_cmd(sock, reacq_turn_dir, REACQ_RPM);
                } else {
                    // 중앙 정렬됨 → 정지 확인
                    send_cmd(sock, DIR_STOP, 0);
                }
            ...
            const char* reacq_st = !line_found ? "NO_LINE" :
                                   !centered    ? (reacq_turn_dir==DIR_LEFT ? "ADJ_L" : "ADJ_R")
                                                : "CENTERED";
            printf("\r[REACQ] %.1f/%.1fs err:%4d %-8s area:%.0f streak:%d dir:%s   ",
                   dt/1000.0f, DUR_REACQ_MS/1000.0f, err2, reacq_st, max_area2,
                   center_streak,
                   (g_reacq_dir==DIR_LEFT)?"L":(g_reacq_dir==DIR_RIGHT)?"R":"ST");
            fflush(stdout);

            // ★ streak 5 달성 시만 S_FOLLOW 전환
            // ★ 타임아웃 와도 streak 미달이면 타이머 리셋 후 계속 정렬
            bool align_done = (line_found && center_streak >= 5);
            bool timeout    = (dt >= DUR_REACQ_MS);

            if (align_done) {
                center_streak = 0;
                g_reacq_dir   = DIR_FORWARD;
                state         = S_REACQ_STABLE;  // ★ 바로 FOLLOW 아닌 안정화 먼저
                state_ts      = t;
                printf("\n[REACQ 완료] 중앙정렬OK → 안정화 구간\n");
            } else if (timeout) {
                state_ts = t;
                printf("\n[REACQ] 타임아웃 리셋 - 계속 정렬중 (streak:%d)\n", center_streak);
            }
            break;
        }
        ...
        case S_REACQ_STABLE: {
            // ★ REACQ 완료 후 800ms 직진으로 라인 위에 완전히 올라탄 뒤 S_FOLLOW
            cmd = DIR_FORWARD;
            if ((t - last_tx) >= 50) {
                send_cmd(sock, DIR_FORWARD, DEFAULT_RPM);
                last_tx = t;
            }
            printf("\r[안정화] %.1f/0.8s 직진 중...     ", dt / 1000.0f);
            fflush(stdout);
            if (dt >= 800) {
                start_ts    = t;
                g_reacq_dir = DIR_FORWARD;
        ...
        case S_REACQUIRE_AFTER_UTURN: {
            ...
            if (dt >= DUR_REACQ_UTURN_MS) {
                start_ts  = t;
                last_junc = 0;   // ★ 복귀 첫 교차로도 쿨다운 없이 바로 인식
                state     = S_FOLLOW;
                state_ts  = t;
                printf("\n[라인 복귀] 추적 시작\n");
            }
        ...
        case S_PRE_UTURN: {
            // ★ 도어 닫힘 후 1.5초 직진 → 유턴 (마찰력 확보 + 라인에서 벗어나기)
            cmd = DIR_FORWARD;
            if ((t - last_tx) >= 50) {
                send_cmd(sock, DIR_FORWARD, DEFAULT_RPM);
                last_tx = t;
            }
            printf("\r[유턴 준비] 직진 중 %.1f/2.5s     ", dt / 1000.0f);
            fflush(stdout);
            if (dt >= 2500) {
                printf("\n[유턴 준비 완료] 유턴 시작!\n");
                state    = S_UTURN;
        ...
        case S_UTURN: {
            cmd = DIR_UTURN;
            if ((t - last_tx) >= 50) {
                send_cmd(sock, DIR_UTURN, UTURN_RPM);
                last_tx = t;
            }

            // ★ 좌/우회전과 동일 방식: 워밍업 1500ms 후 라인+cx 감지 시 완료
            {
                int H2 = frame.rows, W2 = frame.cols;
                int y02 = (int)(H2 * 0.65);
                cv::Mat roi2 = frame(cv::Rect(0, y02, W2, H2 - y02));
                ...
                // ★ 워밍업 3000ms (유턴은 180도, 출발지 라인 잔상 무시)
                bool phase2  = (dt >= 3000);
                // ★ area 상한 제거: 유턴 중 라인이 크게 잡혀도 통과
                bool area_ok = (max_area2 >= 1500);

                // ★ 무게중심: 유턴 완료 후 라인이 화면 중앙 ±40% 범위
                bool cx_ok = false;
                if (area_ok && !ctrs2.empty()) {
                    int best_i = 0; double ba = 0;
                    for (int i = 0; i < (int)ctrs2.size(); i++) {
                    ...
                    if (mm.m00 > 0) {
                        int cx2 = (int)(mm.m10 / mm.m00);
                        cx_ok = (cx2 > W2 * 0.2f) && (cx2 < W2 * 0.8f);
                    }
                }
                bool line_ok = phase2 && area_ok && cx_ok;

                printf("\r[U턴] %.1fs area:%.0f %s      ",
                       dt/1000.0f, max_area2,
                       !phase2 ? "warmup" : (line_ok ? "LINE!" : "searching..."));
                fflush(stdout);

                if (line_ok) {
                    printf("\n[U턴 완료] 라인 감지 (area=%.0f, %.1fs)\n", max_area2, dt/1000.0f);
                    send_cmd(sock, DIR_STOP, 0);
                    last_tx     = t;
                    returning   = true;
                    junc_count  = 0;
                    g_reacq_dir = DIR_UTURN;
                    state       = S_REACQUIRE_AFTER_UTURN;
                    state_ts    = t;
                } else if (dt >= 12000) {
                    printf("\n[U턴 타임아웃] 12초 초과 → 강제 복귀\n");
                    send_cmd(sock, DIR_STOP, 0);
                    last_tx    = t;
                    returning  = true;
                    junc_count = 0;
