...
#define DUR_STRAIGHT_MS       2500   // ★ 2000→2500ms 교차로 완전히 지나친 후 회전
    ...
    while (1) {
        ...
        case S_JUNC_LEFT: {
            ...
            }
            {
                ...
                // ★ 0~1000ms: 워밍업 (교차로 픽셀 벗어나는 시간, 무조건 좌회전)
                // ★ 1000ms~: 라인 감지되면 S_REACQUIRE, 라인 없으면 계속 좌회전
                // ★ 무게중심이 화면 왼쪽 절반에 있어야 진짜 새 라인 (교차로 잔상 아님)
                bool phase2  = (dt >= 2500); // ★ 1500→2500ms 워밍업 연장
                bool area_ok = (max_area2 >= 1500); // ★ area 상한 제거

                // ★ cx 조건 완화: 라인이 화면 90% 이내면 OK (거의 무조건)
                bool cx_ok = false;
                if (area_ok && !ctrs2.empty()) {
                    ...
                    if (mm.m00 > 0) {
                        ...
                        // ★ 좌회전 후: 라인이 화면 어느 위치든 90% 이내면 감지
                        //   (이전 cx < 70% 조건이 너무 엄격해서 18초씩 헤매는 문제 수정)
                        cx_ok = (cx2 < W2 * 0.9f);
                    }
                }
