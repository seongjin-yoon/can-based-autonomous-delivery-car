...
static JuncType detect_junction_hough(const cv::Mat& gray_roi, cv::Mat& vis,
                                       int roi_y_offset, int frame_w)
{
    cv::Mat edges;
    cv::Canny(gray_roi, edges, 50, 150);

    std::vector<cv::Vec4i> lines;
    // HoughLinesP(엣지, 결과, rho, theta, 최소투표수, 최소선길이, 최대갭)
    cv::HoughLinesP(edges, lines, 1, CV_PI / 180.0, 30, 40, 15);

    int horizontal = 0, vertical = 0;

    for (auto& l : lines) {
        int x1 = l[0], y1 = l[1], x2 = l[2], y2 = l[3];
        double angle = fabs(atan2((double)(y2 - y1), (double)(x2 - x1)) * 180.0 / CV_PI);

        if (angle <= 20.0 || angle >= 160.0) {
            // 수평선
            horizontal++;
            cv::line(vis, cv::Point(x1, y1 + roi_y_offset),
                         cv::Point(x2, y2 + roi_y_offset),
                         cv::Scalar(0, 255, 0), 2); // 초록색
    ...
    // 디버그 출력
    printf("\r[Hough] H=%d V=%d        ", horizontal, vertical);
    fflush(stdout);

    // 판단
    // 수평선 2개 이상 + 수직선 1개 이상 → 십자
    if (horizontal >= 2 && vertical >= 1) return J_PLUS;
    // 수평선 2개 이상 + 수직선 없음 → T자
    if (horizontal >= 2 && vertical == 0) return J_T;
...
int main(void) {
    ...
    while (1) {
        ...
        if (state == S_FOLLOW) {
            ...
            {
                int H = frame.rows, W = frame.cols;
                int yj0 = (int)(H * 0.30), yj1 = (int)(H * 0.65);
                ...
                JuncType jt = detect_junction_hough(gray, vis, yj0, W);
                ...
                // streak 진행도 표시
                char streak_str[32];
                snprintf(streak_str, sizeof(streak_str), "streak:%d/%d junc:%d",
