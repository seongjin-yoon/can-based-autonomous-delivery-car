...
static uint8_t        g_reacq_dir   = DIR_FORWARD; // ★ 재탐색 진입 방향 기억
    ...
    while (1) {
        ...
        case S_JUNC_LEFT: {
            ...
            }
            {
                ...
                // ★ 0~800ms: 워밍업 (교차로 픽셀 벗어나는 시간, 무조건 좌회전)
                // ★ 800ms~: 라인 감지되면 S_REACQUIRE, 라인 없으면 계속 좌회전
                bool phase2  = (dt >= 800);
                bool line_ok = phase2 && (max_area2 >= 1500) && (max_area2 < 8000);
        ...
        case S_REACQUIRE: {
            ...
            // ★ 중앙 정렬 완료 시만 S_FOLLOW 전환
            // ★ 타임아웃이 와도 라인 없으면 계속 회전 (직진으로 이탈 방지)
            bool align_done = (line_found && center_streak >= 5);
            bool timeout    = (dt >= DUR_REACQ_MS);

            if (align_done || (timeout && line_found)) {
                // 라인 있는 상태에서 완료
                center_streak = 0;
                start_ts      = t;
                ...
                state_ts      = t;
                printf("\n[REACQ 완료] %s → S_FOLLOW\n",
                       align_done ? "중앙정렬OK" : "타임아웃(라인있음)");
            } else if (timeout && !line_found) {
                // 타임아웃인데 라인 없음 → 타이머 리셋하고 계속 회전
                state_ts = t;
                printf("\n[REACQ] 라인 없음 - 계속 회전중...\n");
            }
            break;
