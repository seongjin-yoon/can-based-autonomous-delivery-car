...
int main(int argc, char* argv[]) {
    ...
    while (1) {
        ...
        case S_JUNC_LEFT: {
            ...
            // ★ 최소 1500ms 회전 후, 라인 감지되면 전환 / 타임아웃 DUR_TURN_MS
            if (dt >= 1500) {
                ...
                if (max_area2 >= 1500) {
                    printf("\n[좌회전 완료] 라인 감지 (area=%.0f, %llums) → 재조정\n",
                           max_area2, (unsigned long long)dt);
                    state    = S_REACQUIRE;
                    state_ts = t;
                } else if (dt >= DUR_TURN_MS) {
                    printf("\n[좌회전 타임아웃] %dms → 재조정\n", DUR_TURN_MS);
                    state    = S_REACQUIRE;
                    state_ts = t;
                }
        ...
        case S_JUNC_RIGHT: {
            ...
            if (dt >= 1500) {
                ...
                if (max_area2 >= 1500) {
                    printf("\n[우회전 완료] 라인 감지 (area=%.0f, %llums) → 재조정\n",
                           max_area2, (unsigned long long)dt);
                    state    = S_REACQUIRE;
                    state_ts = t;
                } else if (dt >= DUR_TURN_MS) {
        ...
        case S_REACQUIRE: {
            ...
            cmd = follow_line(frame, err, vis);
            if ((t - last_tx) >= 50) {
                // ★ 저속으로 라인 정렬 (DEFAULT_RPM → REACQ_RPM)
                send_cmd(sock, cmd, REACQ_RPM);
                last_tx = t;
            }
            printf("\r[REACQ] %.1f/%.1fs err:%d cmd:%s      ",
                   dt/1000.0f, DUR_REACQ_MS/1000.0f, err,
                   (cmd==DIR_FORWARD)?"ST":(cmd==DIR_LEFT)?"L":"R");
            fflush(stdout);
            if (dt >= DUR_REACQ_MS) {
                start_ts = t;
                state    = S_FOLLOW;
                state_ts = t;
                printf("\n[REACQ 완료] S_FOLLOW 전환\n");
            }
            break;
        }
