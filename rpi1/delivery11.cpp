...
int main(int argc, char* argv[]) {
    ...
    while (1) {
        ...
        case S_JUNC_LEFT: {
            ...
            // ★ 최소 500ms 회전 후, 라인 감지되면 전환 / 타임아웃 DUR_TURN_MS
            if (dt >= 500) {
                int turn_err = 0;
                uint8_t turn_dir = follow_line(frame, turn_err, vis);
                bool line_found = (turn_dir != DIR_STOP) ||
                                  (turn_dir == DIR_FORWARD && abs(turn_err) < 80);
                // follow_line이 라인 못찾으면 best_area<500 → DIR_FORWARD + err=0
                // 라인 찾은 경우: contour area >= 500
