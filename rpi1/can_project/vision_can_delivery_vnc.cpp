...
#define PLUS_SIDE_TH   0.35f
#define T_SIDE_TH      0.30f
#define T_MID_TH       0.20f
#define JUNC_STREAK    4
...
int main(void) {
    ...
    while (1) {
        ...
        // 디버그 시각화
        {
            ...
            int y_line=(int)(H*0.65);
            int yj0=(int)(H*0.55), yj1=(int)(H*0.70);
            cv::rectangle(vis,cv::Point(0,y_line),cv::Point(W,H),cv::Scalar(0,255,0),2);
            cv::rectangle(vis,cv::Point(0,yj0),cv::Point(W,yj1),cv::Scalar(255,0,255),2);
            cv::line(vis,cv::Point(W/2,y_line),cv::Point(W/2,H),cv::Scalar(255,0,0),2);
            const char* stname=
            ...
            cv::putText(vis,stname,cv::Point(20,30),cv::FONT_HERSHEY_SIMPLEX,0.9,cv::Scalar(0,255,255),2);
            cv::putText(vis,returning?"RETURN":"GO",cv::Point(20,60),cv::FONT_HERSHEY_SIMPLEX,0.7,cv::Scalar(0,255,255),2);
