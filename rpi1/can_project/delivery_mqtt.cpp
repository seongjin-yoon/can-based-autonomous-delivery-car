...
#define TURN_RPM          150.0f   // 회전 시 RPM (더 높게)
 ...
 *   A: junc0=좌회전
 *   B: junc0=우회전
 *   C: junc0=직진, junc1=좌회전
 *   D: junc0=직진, junc1=우회전
 *
 * returning=true (RETURN):
 *   A: junc0=우회전
 *   B: junc0=좌회전
 *   C: junc0=우회전, junc1=직진
 *   D: junc0=좌회전, junc1=직진
 */
...
static int get_max_junc(int dest) {
    if (dest == 1 || dest == 2) return 1; // A, B: 교차로 1개
    return 2;                              // C, D: 교차로 2개
}
...
int main(int argc, char* argv[]) {
    ...
    while (1) {
        ...
        case S_JUNC_LEFT:
            cmd = DIR_LEFT;
	    send_cmd(sock, DIR_LEFT, TURN_RPM);  // 회전 RPM 따로 적용
        ...
        case S_DELIVER_WAIT:
            ...
            if (dt >= DELIVER_WAIT_MS) {
                ...
                state_ts = t;
                printf("\n[U턴] 복귀 시작\n");
                mqtt.send_arrived(dest); // 출발 알림 재사용
            }
            break;
