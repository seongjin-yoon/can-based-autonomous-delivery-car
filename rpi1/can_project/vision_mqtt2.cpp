 ...
 * [수정] 회전 RPM 낮춤 (250 → 150), 회전 시간 늘림 (750 → 1200ms)
 */
...
// ===================== 주행 파라미터 =====================
#define DEFAULT_RPM       250.0f   // 직진 RPM
#define TURN_RPM          150.0f   // 회전 RPM (낮춤)
...
// ===================== 상태 지속 시간 =====================
#define DUR_JUNC_STOP_MS 1000    // 교차로 정지 1초
#define DUR_STRAIGHT_MS  500     // 직진 500ms
#define DUR_TURN_MS      1200    // 회전 시간 (750 → 1200ms)
...
int main(int argc, char* argv[]) {
    ...
    printf("직진RPM:%.0f  회전RPM:%.0f  회전시간:%dms\n",
           (float)DEFAULT_RPM, (float)TURN_RPM, DUR_TURN_MS);
    printf("키: q=종료 e=E-Stop 1/2/3/4=수동목적지\n");
    printf("브로커: %s\n\n", broker_host.c_str());
    ...
    if (!mqtt_ok) printf("[MQTT] 오프라인 모드\n");
    ...
    while (1) {
        ...
        case S_FOLLOW: {
            ...
            if ((t - last_tx) >= 200) {
                ...
                printf("\r[%s err=%4d] DEST:%d JUNC:%d", d, err, dest, junc_count);
                fflush(stdout);
            }
            break;
        }
        ...
        // CAN 송신 (50ms마다) - 회전 상태 제외 (위에서 직접 처리)
        if ((t - last_tx) >= 50) {
            if (state != S_JUNC_LEFT && state != S_JUNC_RIGHT && state != S_JUNC_STRAIGHT) {
        ...
        char info[48];
        snprintf(info, sizeof(info), "J:%d ST_RPM:%.0f TN_RPM:%.0f",
                 junc_count, (float)DEFAULT_RPM, (float)TURN_RPM);
