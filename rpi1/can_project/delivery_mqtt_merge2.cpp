...
#define DUR_TURN_MS       2500
#define DUR_UTURN_MS      9000   // ★ 유턴 3초
#define DUR_REACQ_MS      1000   // ★ 유턴 후 직진 1초 (라인 무시)
...
typedef enum {
    ...
    S_JUNC_RIGHT,
    S_REACQUIRE,
    S_REACQUIRE_AFTER_UTURN,  // ★ 유턴 후 직진 전용 상태
    S_DELIVER_WAIT,
    S_UTURN,
    S_FINISHED
} State;
...
int main(int argc, char* argv[]) {
    ...
    while (1) {
        ...
        case S_REACQUIRE_AFTER_UTURN: {
            ...
            printf("\r[유턴 후 직진 중] %llums / %dms", (unsigned long long)dt, DUR_REACQ_MS);
            fflush(stdout);
            if (dt >= DUR_REACQ_MS) {
                start_ts   = t;
                last_junc  = t;
        ...
        char info[64];
        snprintf(info, sizeof(info), "J:%d TURN:%dms UTN:%dms",
                 junc_count, DUR_TURN_MS, DUR_UTURN_MS);
