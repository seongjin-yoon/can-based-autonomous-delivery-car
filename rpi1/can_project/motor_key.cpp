 ...
 *
 * 키:
 *   1/2/3/4 = 목적지 A/B/C/D 출발
 *   w/a/s/d = 수동 전진/좌/정지/우
 *   e = E-Stop  q = 종료
 */
...
int main(int argc, char* argv[]) {
    ...
    printf("목적지: 1=A 2=B 3=C 4=D\n");
    printf("수동:   w=전진 a=좌회전 d=우회전 s=정지\n");
    printf("기타:   q=종료 e=E-Stop\n");
    printf("브로커: %s\n\n", broker_host.c_str());
    ...
    // ★ 수동 제어용
    uint8_t  manual_cmd = DIR_STOP;
    uint64_t last_manual_tx = 0;

    printf("명령 대기 중... (w/a/s/d로 수동 테스트 가능)\n");
    ...
    while (1) {
        ...
        // ★ 수동 제어 상태: 50ms마다 재전송
        case S_MANUAL:
            if ((t - last_manual_tx) >= 50) {
                float rpm = (manual_cmd == DIR_STOP) ? 0.0f :
                            (manual_cmd == DIR_FORWARD) ? DEFAULT_RPM : TURN_RPM;
                send_cmd(sock, manual_cmd, rpm);
                last_manual_tx = t;
                last_tx = t;
            }
            cmd = manual_cmd;
        ...
        // CAN 공통 송신 (S_MANUAL은 위에서 직접 처리)
        if (state != S_MANUAL && (t - last_tx) >= 50) {
