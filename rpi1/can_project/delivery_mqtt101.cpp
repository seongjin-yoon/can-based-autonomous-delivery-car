...
// ===================== 주행 파라미터 =====================
#define DEFAULT_RPM       50.0f   // 직진 RPM
#define TURN_RPM          150.0f  // 회전 RPM (STM32 고정PWM이라 실제 무시됨)
#define STRAIGHT_DEADBAND 40      // 라인 중심 오차 허용범위

// ===================== 교차로 감지 파라미터 =====================
#define JUNC_STREAK       2       // 연속 감지 횟수
#define JUNC_START_DELAY  3000    // 출발 후 교차로 감지 시작까지 딜레이(ms)
#define JUNC_COOLDOWN_MS  3000    // 교차로 처리 후 쿨다운(ms)
#define HIGH_TH           0.40f
#define HIGH_TH_ONE       0.65f
#define MID_TH            0.40f

// ===================== 상태 지속 시간 =====================
#define DUR_JUNC_STOP_MS  500     // 교차로 진입 후 정지 시간 (0.5초)
#define DUR_STRAIGHT_MS   1500    // 직진 통과 시간 (1.5초)
#define DUR_TURN_MS       3000    // 회전 시간 (3초)
#define DUR_UTURN_MS      1500    // U턴 시간 (1.5초)
#define DUR_REACQ_MS      500     // 라인 재탐색 시간 (0.5초)
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        if (std::string(argv[i]) == "--no-display") no_display = true;
        else broker_host = argv[i];
    }

    printf("=== 배달 차량 (라인트레이싱 + MQTT) ===\n");
    printf("목적지: A=1 B=2 C=3 D=4\n");
    printf("키: q=종료 e=E-Stop 1/2/3/4=수동목적지설정\n");
    printf("브로커: %s  디스플레이: %s\n\n",
           broker_host.c_str(), no_display ? "OFF" : "ON");
    ...
    // 카메라 스레드 시작
    std::thread cam_th(camera_thread_func, std::ref(cap));
    cam_th.detach();
    ...
    while (1) {
        // 카메라 프레임 가져오기 (스레드에서)
        if (!g_frame_ready) { usleep(1000); continue; }
        cv::Mat frame;
        {
            std::lock_guard<std::mutex> lock(g_frame_mutex);
            frame = g_frame.clone();
        ...
        cv::putText(vis, std::string("DEST:")+dn2+(returning?" RET":" GO"),
                    cv::Point(10, 45),
