 ...
 *
 * [목적지별 경로]
 *   A: 1번교차로 좌회전 → ArUco ID=1 → 유턴 → 우회전 → 복귀
 *   B: 1번교차로 우회전 → ArUco ID=2 → 유턴 → 좌회전 → 복귀
 *   C: 1번교차로 직진 → 2번교차로 좌회전 → ArUco ID=3 → 유턴 → 우회전 → 직진 → 복귀
 *   D: 1번교차로 직진 → 2번교차로 우회전 → ArUco ID=4 → 유턴 → 좌회전 → 직진 → 복귀
 *
 * 컴파일:
...
// ★ 출발 후 교차로 무시 시간 (ms) - 출발 직후 오인식 방지
...
int main(int argc, char* argv[]) {
    ...
    uint64_t last_tx    = 0;
    uint64_t start_ts   = now_ms();      // ★ 출발 시각 기록용
    int      streak     = 0;
    int      junc_count = 0;
    int      dest       = 0;
    ...
    while (1) {
        ...
        case S_FOLLOW: {
            ...
            // ★ 출발 후 JUNC_START_DELAY ms 동안은 교차로 감지 무시
            ...
            // ★ 출발 딜레이 남은 시간 표시
            int delay_remain = (int)(JUNC_START_DELAY - (t - start_ts)) / 1000 + 1;
            char dbg[80];
            if (!start_ok) {
                snprintf(dbg, sizeof(dbg), "S=%d/%d J=%d %s L=%.2f M=%.2f R=%.2f [딜레이 %ds]",
                         streak, JUNC_STREAK, junc_count,
                         returning ? "RET" : "GO", rL, rM, rR, delay_remain);
            } else {
        ...
        // CAN 송신 50ms마다 (회전/직진 상태는 위에서 직접 송신)
        if ((t - last_tx) >= 50) {
        ...
        char info[48];
        snprintf(info, sizeof(info), "J:%d RPM:%.0f TURN:%dms",
                 junc_count, g_rpm, DUR_TURN_MS);
        cv::putText(vis, info, cv::Point(10, 68),
                    cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255,200,0), 1);
	
	// 교차로 ROI 이진화 화면 표시
	int H2 = frame.rows, W2 = frame.cols;
	int yj0_dbg = (int)(H2 * 0.40), yj1_dbg = (int)(H2 * 0.62);
	cv::Mat jroi_dbg = frame(cv::Rect(0, yj0_dbg, W2, yj1_dbg - yj0_dbg));
	cv::Mat gray_dbg, blur_dbg, bin_dbg;
	cv::cvtColor(jroi_dbg, gray_dbg, cv::COLOR_BGR2GRAY);
	cv::GaussianBlur(gray_dbg, blur_dbg, cv::Size(5,5), 0);
	cv::threshold(blur_dbg, bin_dbg, 0, 255, cv::THRESH_BINARY_INV | cv::THRESH_OTSU);
	cv::imshow("Junction ROI", bin_dbg);
