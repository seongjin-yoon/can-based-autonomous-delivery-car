 ...
 *
 * 컴파일:
 *   g++ delivery_mqtt_merge6.cpp -o delivery_merge6 \
 *       $(pkg-config --cflags --libs opencv4) \
 *       -lmosquittopp -lpthread
 * 실행:
 *   ./delivery_merge6 [broker_ip]
 */
...
#define JUNC_COOLDOWN_MS  2000   // ★ 3500 → 2000
...
#define DUR_TURN_MS           3000   // 라인감지 기반이라 fallback용으로만 사용
#define DUR_UTURN_MS          3000
#define DUR_REACQ_MS          1000   // ★ 500 → 1000
#define DUR_REACQ_UTURN_MS    1000
#define TURN_MIN_MS           300    // ★ 최소 회전 시간 (라인 오인식 방지)
#define TURN_LINE_ERR_TH      80     // ★ 라인 감지 판단 err 임계값
...
static uint8_t follow_line(const cv::Mat& frame, int& err_out, cv::Mat& vis) {
    ...
    if (best < 0 || best_area < 500) { err_out = 0; return DIR_STOP; }  // ★ 라인 없으면 DIR_STOP
...
int main(int argc, char* argv[]) {
    ...
    while (1) {
        ...
        case S_JUNC_LEFT:
            ...
            if (dt >= TURN_MIN_MS) {  // 최소 300ms 회전 후
                int err = 0;
                cv::Mat vis_tmp = vis.clone();
                uint8_t line_cmd = follow_line(frame, err, vis_tmp);
                // 라인이 감지되고 중앙 근처에 있으면 완료
                if (line_cmd != DIR_STOP && abs(err) < TURN_LINE_ERR_TH) {
                    state    = S_REACQUIRE;
                    state_ts = t;
                    printf("\n[좌회전 완료] 라인 감지 err=%d\n", err);
                }
            }
            // fallback: 시간 초과 시에도 넘어감
            if (dt >= DUR_TURN_MS) {
                state    = S_REACQUIRE;
                state_ts = t;
                printf("\n[좌회전 완료] 타임아웃 fallback\n");
            }
            break;
        ...
        case S_REACQUIRE: {
            int err = 0;
            cmd = follow_line(frame, err, vis);
            if ((t - last_tx) >= 100) {          // ★ 50 → 100ms
                send_cmd(sock, cmd, 20.0f);        // ★ DEFAULT_RPM → 20
                last_tx = t;
            }
            if (dt >= DUR_REACQ_MS) {            // ★ 1000ms
                start_ts = t;
                state    = S_FOLLOW;
                state_ts = t;
            }
