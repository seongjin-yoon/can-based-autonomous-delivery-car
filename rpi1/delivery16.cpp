/*
 * delivery_mqtt_merge11.cpp
 * 라인트레이싱 + ArUco + MQTT 통합 버전
 *
 * 수정사항 (merge11 - 후진 귀환):
 *   1. S_UTURN / S_REACQUIRE_AFTER_UTURN 제거
 *   2. S_RETURN_BACK 추가 — 후진하며 교차로 감지
 *   3. get_junction_action() returning 테이블 재정의
 *      A: junc0 → 좌회전
 *      B: junc0 → 우회전
 *      C: junc0 → 좌회전, junc1 → 직진(잠깐 정지 후 S_FOLLOW)
 *      D: junc0 → 우회전, junc1 → 직진(잠깐 정지 후 S_FOLLOW)
 *   4. S_JUNC_STOP: returning + FORWARD → S_REACQUIRE 직행 (2초 직진 생략)
 *   5. g_door_closed → S_RETURN_BACK 진입
 *
 * 컴파일:
 *   g++ delivery_mqtt_merge11.cpp -o delivery11 \
 *       $(pkg-config --cflags --libs opencv4) \
 *       -lmosquittopp -lpthread
 * 실행:
 *   ./delivery11 [broker_ip]
 */
...
// ===================== 주행 파라미터 =====================
#define DEFAULT_RPM       40.0f // 기본 RPM (라인 추적 시)
#define RETURN_BACK_RPM   30.0f // 후진 시 낮은 RPM으로 설정하여 안정적으로 후진하도록 함
#define TURN_RPM          130.0f // 회전 시 RPM (직진보다 높게 설정하여 빠르게 회전)
...
#define DUR_REACQ_MS          1500
#define DUR_REACQ_UTURN_MS    1000
#define REACQ_RPM             30.0f

// 후진 귀환: 교차로 감지 시작 전 최소 후진 시간 (교차로 바로 위에서 시작 시 오감지 방지)
#define RETURN_BACK_DELAY_MS  3000
...
typedef enum {
    ...
    S_REACQUIRE,
    S_DELIVER_WAIT,
    S_RETURN_BACK,      // ★ 후진하며 교차로 감지 (구 S_UTURN 대체)
    S_FINISHED
} State;
...
static uint8_t        g_reacq_dir   = DIR_FORWARD;
...
static uint8_t get_junction_action(int dest, int junc_count, bool returning) {
    if (!returning) {
        ...
        // ★ 후진 귀환 로직
        switch (dest) {
            case 1: if (junc_count == 0) return DIR_LEFT;   break;  // 후진→좌회전
            case 2: if (junc_count == 0) return DIR_RIGHT;  break;  // 후진→우회전
            case 3:
                if (junc_count == 0) return DIR_LEFT;               // 후진→좌회전
                if (junc_count == 1) return DIR_FORWARD;            // 교차로 정지 후 직진 추적
                break;
            case 4:
                if (junc_count == 0) return DIR_RIGHT;              // 후진→우회전
                if (junc_count == 1) return DIR_FORWARD;            // 교차로 정지 후 직진 추적
                break;
        }
    }
    ...
    if (select(sock + 1, &fds, NULL, NULL, &tv) > 0) {
        if (read(sock, &frame, sizeof(frame)) > 0) {
            if (frame.can_id == CAN_DOOR_STATUS && frame.data[0] == 0x00) {
                printf("\n[CAN RX] 0x301=0x00 도어 닫힘 → 후진 귀환 시작!\n");
                g_door_closed = true;
            }
                ...
                printf("\n[CAN RX] 0x302=0x02 오배달/미수령 → 후진 귀환 시작!\n");
                g_door_closed = true;
            }
...
// ===================== 교차로 감지 공통 처리 (이진화 결과 반환) =====================
static JuncType detect_junction_from_frame(const cv::Mat& frame,
                                            float& rL, float& rM, float& rR) {
    int H = frame.rows, W = frame.cols;
    ...
    return classify_junction(bin, rL, rM, rR);
}

// ===================== 하단 ROI 라인 면적 계산 공통 처리 =====================
static double get_line_area(const cv::Mat& frame) {
    int H = frame.rows, W = frame.cols;
    int y0 = (int)(H * 0.65);
    cv::Mat roi = frame(cv::Rect(0, y0, W, H - y0));
    cv::Mat gray, blur, bin;
    ...
    double max_area = 0;
    for (auto& c : ctrs) max_area = std::max(max_area, cv::contourArea(c));
    return max_area;
}
...
int main(int argc, char* argv[]) {
    ...
    while (1) {
        ...
        case S_FOLLOW: {
            ...
            // 교차로 감지
            float rL, rM, rR;
            JuncType jt = detect_junction_from_frame(frame, rL, rM, rR);
            ...
            // 라인 추적
            int err = 0;
            uint8_t cmd = follow_line(frame, err, vis);
            if ((t - last_tx) >= 50) {
        ...
        case S_JUNC_STOP:
            ...
            if (dt >= DUR_JUNC_STOP_MS) {
                ...
                if (g_next_action == DIR_FORWARD) {
                    if (returning) {
                        // ★ 귀환 중 두 번째 교차로(C/D): 정지만 하고 바로 S_FOLLOW
                        // (2초 직진 생략 — 라인 탐색하며 ArUco 0 찾기)
                        g_reacq_dir = DIR_FORWARD;
                        state    = S_REACQUIRE;
                        state_ts = t;
                        printf("\n[귀환 교차로 통과] 정지 후 라인 탐색 재개\n");
                    } else {
                        state = S_JUNC_STRAIGHT;
                        state_ts = t;
                        printf("\n[행동 시작] %s (%dms)\n", an, DUR_STRAIGHT_MS);
                    }
                } else if (g_next_action == DIR_LEFT) {
                    state = S_JUNC_LEFT;
                    state_ts = t;
                    printf("\n[행동 시작] %s\n", an);
                } else {
        ...
        case S_JUNC_LEFT: {
            ...
            double max_area2 = get_line_area(frame);
            bool phase2  = (dt >= 1000);
            bool area_ok = (max_area2 >= 1500) && (max_area2 < 5000);

            // 무게중심이 화면 왼쪽~중앙에 있어야 함 (좌회전 후 새 라인)
            bool cx_ok = false;
            if (area_ok) {
        ...
        case S_REACQUIRE: {
            ...
            printf("\r[REACQ] %.1fs err:%4d area:%.0f streak:%d dir:%s   ",
                   dt/1000.0f, err2, max_area2, center_streak,
            ...
            if (align_done) {
                center_streak = 0;
                start_ts      = t;
                last_junc     = t;   // 쿨다운 시작 (방금 교차로 통과했으므로)
                g_reacq_dir   = DIR_FORWARD;
                state         = S_FOLLOW;
                state_ts      = t;
                printf("\n[REACQ 완료] 중앙정렬OK → S_FOLLOW\n");
            } else if (timeout) {
                state_ts = t;  // 타이머 리셋, 계속 정렬
                printf("\n[REACQ] 타임아웃 리셋 (streak:%d)\n", center_streak);
            }
            break;
        }
        ...
        case S_DELIVER_WAIT:
            ...
            }
            {
                ...
                printf("\r[PIN 인증 대기] %llus 경과 (도어 닫히면 후진 귀환)  ",
        ...
        case S_RETURN_BACK: {
            // 후진 명령 송신
            if ((t - last_tx) >= 50) {
                send_cmd(sock, DIR_BACKWARD, RETURN_BACK_RPM);
                last_tx = t;
            }

            // RETURN_BACK_DELAY_MS 이후부터만 교차로 감지
            // (목적지 바로 위에서 시작 시 잔상 오감지 방지)
            bool delay_ok = (dt >= RETURN_BACK_DELAY_MS);

            float rL, rM, rR;
            JuncType jt = delay_ok
                ? detect_junction_from_frame(frame, rL, rM, rR)
                : J_NONE;

            if (delay_ok && jt != J_NONE) streak++;
            else if (!delay_ok)            streak = 0;
            else                           streak = std::max(0, streak - 1);

            printf("\r[후진 귀환] %.1fs streak:%d/%d L=%.2f M=%.2f R=%.2f %s   ",
                   dt/1000.0f, streak, JUNC_STREAK, rL, rM, rR,
                   delay_ok ? "" : "(딜레이)");
            fflush(stdout);

            if (delay_ok && streak >= JUNC_STREAK) {
                streak    = 0;
                last_junc = t;
                ...
                const char* an = (action==DIR_FORWARD)?"직진":
                                 (action==DIR_LEFT)?"좌회전":"우회전";
                const char* dn = (dest==1)?"A":(dest==2)?"B":(dest==3)?"C":"D";
                printf("\n[귀환 교차로 감지] DEST:%s junc:%d → %s (L=%.2f M=%.2f R=%.2f)\n",
                       dn, junc_count, an, rL, rM, rR);
        ...
        const char* stname =
            ...
            (state == S_RETURN_BACK)  ? "RETURN_BACK" : "FINISH";
        ...
        char info[64];
        snprintf(info, sizeof(info), "J:%d DOOR:%s",
                 junc_count, g_door_closed ? "CLOSED" : "OPEN");
