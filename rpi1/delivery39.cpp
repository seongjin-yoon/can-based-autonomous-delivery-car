/*
 * delivery_mqtt_merge12.cpp
 * 라인트레이싱 + ArUco + MQTT 통합 버전
 *
 * 수정사항 (merge12 - merge10 기반):
 *   1. S_PRE_UTURN / S_UTURN / S_REACQUIRE_AFTER_UTURN 제거
 *   2. S_RETURN_BACK 추가 — 후진하며 교차로 감지
 *   3. get_junction_action() returning 테이블 재정의
 *      A(1): junc0 → 좌회전
 *      B(2): junc0 → 우회전
 *      C(3): junc0 → 좌회전, junc1 → 직진(정지 후 S_REACQUIRE)
 *      D(4): junc0 → 우회전, junc1 → 직진(정지 후 S_REACQUIRE)
 *   4. S_JUNC_STOP: returning + FORWARD → S_REACQUIRE 직행
 *   5. DIR_LEFT(3) ↔ DIR_RIGHT(4) 스왑 (실제 하드웨어 방향 보정)
 *   6. RETURN_BACK_DELAY_MS=3000 (후진 시작 후 교차로 감지 딜레이)
 *
 * 컴파일:
 *   g++ delivery_mqtt_merge12.cpp -o delivery12 \
 *       $(pkg-config --cflags --libs opencv4) \
 *       -lmosquittopp -lpthread
 * 실행:
 *   ./delivery12 [broker_ip]
 */
...
// ===================== 모터 명령 코드 =====================
// ★ DIR_LEFT/RIGHT 스왑 (하드웨어 방향 보정)
#define DIR_STOP      0
#define DIR_FORWARD   1
#define DIR_BACKWARD  2
#define DIR_LEFT      4   // ★ 스왑: 원래 3이었으나 실제 우회전 → 4로 변경
#define DIR_RIGHT     3   // ★ 스왑: 원래 4이었으나 실제 좌회전 → 3으로 변경
#define DIR_UTURN     5

// ===================== 주행 파라미터 =====================
#define DEFAULT_RPM           40.0f
#define RETURN_BACK_RPM       40.0f   // ★ 후진 속도
#define TURN_RPM              150.0f  // 회전 시 RPM (직진보다 높게 설정하여 빠르게 회전)
...
// ★ 후진 귀환: 교차로 감지 시작 전 딜레이 (목적지 바로 위 오감지 방지)
 ...
 * B(2): junc0 → 우   junc0 → 우
 * C(3): junc0 → 직   junc0 → 좌
 *       junc1 → 좌   junc1 → 직(정지 후 S_REACQUIRE)
 * D(4): junc0 → 직   junc0 → 우
 *       junc1 → 우   junc1 → 직(정지 후 S_REACQUIRE)
 */
    ...
    if (!returning) {
        ...
        switch (dest) {
            ...
            case 3:
                ...
                if (junc_count == 1) return DIR_FORWARD;    // 정지 후 S_REACQUIRE
                break;
            case 4:
...
// ★ 교차로 감지 공통 헬퍼 (S_FOLLOW / S_RETURN_BACK 공용)
...
int main(int argc, char* argv[]) {
    ...
    while (1) {
        ...
        case S_JUNC_STOP:
            ...
            if (dt >= DUR_JUNC_STOP_MS) {
                ...
                if (g_next_action == DIR_FORWARD) {
                    if (returning) {
                        // ★ 귀환 중 두 번째 교차로(C/D): 정지만 하고 S_REACQUIRE 직행
        ...
        case S_REACQUIRE: {
            ...
            bool align_done = (line_found && center_streak >= 3);
