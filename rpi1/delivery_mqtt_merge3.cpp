...
#define DUR_UTURN_MS          3000    // 유턴 3초
#define DUR_REACQ_MS          500     // 교차로 후 라인 재탐색
#define DUR_REACQ_UTURN_MS    1000    // 유턴 후 직진 (흰배경 무시)
#define DELIVER_WAIT_MS       10000   // 도어 닫힘 수신 없을 때 최대 대기
...
typedef enum {
    ...
    S_JUNC_RIGHT,
    S_REACQUIRE,
    S_REACQUIRE_AFTER_UTURN,  // 유턴 후 직진 전용 (라인 무시)
    S_DELIVER_WAIT,
    S_UTURN,
    S_FINISHED
} State;
...
static std::atomic<bool> g_door_closed(false);  // ★ 적재함 도어 닫힘 플래그
...
void recv_can(int sock) {
    ...
    if (select(sock + 1, &fds, NULL, NULL, &tv) > 0) {
        if (read(sock, &frame, sizeof(frame)) > 0) {
            // ★ 0x301 = 0x00: 도어 닫힘 → 유턴 트리거
            if (frame.can_id == 0x301 && frame.data[0] == 0x00) {
                printf("\n[CAN RX] 0x301=0x00 적재함 도어 닫힘 → 유턴 시작!\n");
                g_door_closed = true;
            }
            // 0x302 = 0x01: 인증 성공 (로그만)
            else if (frame.can_id == 0x302 && frame.data[0] == 0x01) {
                printf("\n[CAN RX] 0x302=0x01 인증 성공 (도어 열림)\n");
            }
            // 0x302 = 0x02: 오배달/미수령 (로그만)
            else if (frame.can_id == 0x302 && frame.data[0] == 0x02) {
                printf("\n[CAN RX] 0x302=0x02 오배달/미수령 → 즉시 유턴\n");
                g_door_closed = true;  // 오배달도 유턴 트리거
            }
        }
    }
}
    ...
    while (1) {
        ...
        case S_FOLLOW: {
            ...
            if (!returning && dest > 0 && aid == get_target_aruco(dest)) {
                ...
                state_ts = t;
                g_door_closed = false;  // 플래그 초기화
                mqtt.send_arrived(dest);
                printf("[대기] 적재함 도어 닫힘 대기 (최대 %d초)\n",
                       DELIVER_WAIT_MS / 1000);
                break;
            }
            if (returning && aid == 0) {
                printf("\n[복귀 완료] ArUco ID=0 감지!\n");
        ...
        case S_REACQUIRE_AFTER_UTURN: {
            // ★ 유턴 후 직진만 - 흰 배경 무시, 라인트레이싱 안 함
            cmd = DIR_FORWARD;
            if ((t - last_tx) >= 50) {
                send_cmd(sock, DIR_FORWARD, DEFAULT_RPM);
                last_tx = t;
            }
            printf("\r[유턴 후 직진 중] %.1f초 / %.1f초",
        ...
        case S_DELIVER_WAIT:
            ...
            }
            {
                ...
                printf("\r[배달 대기] %d초 남음 (도어 닫히면 자동 출발)  ", remain);
                fflush(stdout);
                char ws[48];
            ...
            // ★ 도어 닫힘 수신 OR 타임아웃
            if (g_door_closed || dt >= DELIVER_WAIT_MS) {
                if (g_door_closed)
                    printf("\n[도어 닫힘] 유턴 시작!\n");
                else
                    printf("\n[타임아웃] %d초 경과 → 유턴 시작\n", DELIVER_WAIT_MS/1000);
