...
통계청 「온라인쇼핑동향조사」에 따르면 음식 배달 서비스의 온라인 거래액은  
2017년 이후 지속적으로 증가했으며, 특히 COVID-19 기간 동안 급격한 성장을 보였다.

이러한 성장으로 인해 배달 시스템은 점점 더 많은 요청을 처리해야 하는 구조로 발전하고 있으며,  
단일 중앙 시스템에 의존하는 구조는 장애 발생 시 전체 서비스 중단이라는 리스크를 가진다.

반면, 실제 차량 전장 시스템은 기능별 ECU를 분리한 분산 구조를 사용하여  
시스템 안정성과 확장성을 확보한다.

👉 본 프로젝트는 이러한 산업 구조를 반영하여,  
RC카 환경에서 CAN 기반 분산 ECU 아키텍처를 설계하고 검증하는 것을 목표로 한다.

> "단순 기능 구현이 아니라, 실제 시스템 구조를 설계하는 프로젝트"
...
[![Demo Video](https://img.youtube.com/vi/Z4IQcvlYzos/0.jpg)](https://youtu.be/Z4IQcvlYzos)
