    ...
    : QWidget(parent)
{
    ...
    // TODO: [MQTT - 팀원 수정] 아래 테스트 타이머 제거하고 MQTT 수신 콜백으로 교체
    ...
    connect(timer, &QTimer::timeout, this, [this, progress, timer](){
        *progress += 0.01f;
        if (*progress >= 1.0f) {
            *progress = 1.0f;
            timer->stop();  // 여기서 멈춰야 함
            updateState(DeliveryState::ARRIVED, *progress);
        } else {
            updateState(DeliveryState::DRIVING, *progress);
        }
    });
    timer->start(200); // 200ms마다 1% 증가
    ...
    // TODO: [MQTT - 팀원 수정] 테스트용 버튼 제거 후 MQTT AUTH_SUCCESS 수신 시 updateState 호출
...
// TODO: [MQTT - 팀원 수정] 외부에서 이 함수를 호출하여 상태 업데이트
// 예시: monitor->updateState(DeliveryState::DRIVING, 0.3f);
// 예시: monitor->updateState(DeliveryState::ARRIVED, 1.0f);
// 예시: monitor->updateState(DeliveryState::AUTH_SUCCESS, 1.0f);
// 예시: monitor->updateState(DeliveryState::RETURNING, 1.0f);
void MonitorWindow::updateState(DeliveryState state, float progress)
{
    ...
    if (state == DeliveryState::ARRIVED) {
        startCountdown();
        btn_collect->setVisible(true); // 테스트용 완료 버튼 생성 MQTT 팀원
