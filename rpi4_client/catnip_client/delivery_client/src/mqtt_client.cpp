...
void MQTTClient::MQTTCallback::message_arrived(mqtt::const_message_ptr msg) {
    ...
    qDebug() << "📨 메시지 수신 토픽:" << topic;

    // delivery/vehicle/+/1to3 토픽 처리
    if (topic.contains("/1to3")) {
        QJsonDocument doc = QJsonDocument::fromJson(payload.toUtf8());
        if (!doc.isNull() && doc.isObject()) {
            QJsonObject obj = doc.object();
            QString status   = obj["status"].toString();
            QString order_id = obj["order_id"].toString();

            if (status == "completed") {
                qDebug() << "✅ 배달 완료 수신 order_id:" << order_id;
                emit m_parent->delivery_completed(order_id);
            }
        }
    }
}
...
MQTTClient::MQTTClient(const QString& broker_address,
    ...
    try {
        ...
        QObject::connect(this, &MQTTClient::delivery_completed,
                         this, [this](const QString& order_id) {
                             QMessageBox::information(nullptr,
                                                      "배달 완료",
                                                      QString("주문 %1\n배달이 완료되었습니다!").arg(order_id));
                         });
...
void MQTTClient::subscribe_topics() {
    ...
    try {
        ...
        m_mqtt_client->subscribe("delivery/vehicle/+/1to3", 1);  // ★ 추가
